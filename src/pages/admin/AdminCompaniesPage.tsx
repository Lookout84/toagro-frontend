...
const AdminCompaniesPage: React.FC = () => {
  ...
  const [pendingDocumentsFilter, setPendingDocumentsFilter] =
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        if (pendingDocumentsFilter) params.hasPendingDocuments = true;

        const response = await adminAPI.getCompanies(params);
        // ОНОВЛЕНО: враховуємо вкладеність data.data
        ...
        setCompanies([]);
        setError(
          "Помилка при завантаженні списку компаній. Спробуйте знову пізніше."
        );
      } finally {
        setLoading(false);
      }
  ...
  const fetchCompanyDocuments = async (companyId: number) => {
    try {
      ...
      setCompanyDocuments(Array.isArray(response.data) ? response.data : []);
    } catch {
      setCompanyDocuments([]);
  ...
  const handlePendingDocumentsFilterChange = (
  ...
  const handleRejectCompany = async () => {
    ...
    try {
      setActionLoading(true);
      await adminAPI.rejectCompany(selectedCompany.id, {
        reason: rejectReason,
      });
      setCompanies((prev) =>
        prev.map((company) =>
  ...
  const getDocumentStatus = (status: string) => {
    switch (status) {
      case "VERIFIED":
        return (
          <Badge color="green" icon={<Check size={14} />}>
            Підтверджено
          </Badge>
        );
      case "REJECTED":
        return (
          <Badge color="red" icon={<X size={14} />}>
            Відхилено
          </Badge>
        );
      case "PENDING":
      default:
        return (
          <Badge color="yellow" icon={<AlertTriangle size={14} />}>
            Очікує перевірки
          </Badge>
        );
    }
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-4 text-sm text-gray-600 text-center">
          Показано {Array.isArray(companies) ? companies.length : 0} компаній з{" "}
          {totalItems}
        </div>
                ...
                <div className="md:w-2/3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-900">
                        {getCompanySize(selectedCompany.size)}
                      </p>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6">
                    <div>
                      ...
                      <p className="text-gray-900">
                        {selectedCompany.user.firstName}{" "}
                        {selectedCompany.user.lastName}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">
              ...
              <Tabs activeTab={activeTab} onChange={setActiveTab}>
                <Tab title="Документи">
                  {actionLoading ? (
                    ...
                    <div>
                      {Array.isArray(companyDocuments) &&
                      companyDocuments.length > 0 ? (
                                    ...
                                    {document.verificationStatus ===
                                      ...
                                      <>
                                        <Button
                                          ...
                                          size="small"
                                          onClick={() =>
                                            handleVerifyDocument(document.id)
                                          }
                                          className="mr-2"
                        ...
                        <div className="text-center py-8">
                          ...
                          <p className="text-gray-600 max-w-md mx-auto">
                            Ця компанія ще не завантажила жодного документа для
                            верифікації.
                          </p>
                        </div>
                      )}
            ...
            <div className="p-6">
              <div className="mb-6">
                ...
                <p className="text-gray-600 mb-2">
                  Ви збираєтесь верифікувати компанію{" "}
                  <strong>{selectedCompany.companyName}</strong>.
                </p>
                <p className="text-gray-600">
                  Верифікована компанія отримає доступ до всіх функцій
                  платформи. Це дія означає, що:
                </p>
                <ul className="list-disc pl-5 mt-2 text-gray-600 space-y-1">
                  <li>
                    Ви перевірили документи компанії та підтверджуєте їх
                    достовірність
                  </li>
                  <li>
                    Компанія отримає спеціальну відмітку
                    &quot;Верифіковано&quot; у профілі
                  </li>
                  <li>
                    Компанія отримає доступ до додаткових функцій платформи
                  </li>
                </ul>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="mb-4">
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Ця інформація буде надіслана компанії для виправлення
                    проблем.
                  </p>
                </div>
              </div>
