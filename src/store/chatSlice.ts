  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Помилка завантаження бесід",
      );
    }
  },
);
...
export const sendMessage = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "Помилка відправки повідомлення",
      );
    }
  },
);
...
export const markConversationAsRead = createAsyncThunk(
  ...
  async (userId: number, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message ||
          "Помилка позначення повідомлень як прочитаних",
      );
    }
  },
);
    ...
    addLocalMessage: (state, action: PayloadAction<Message>) => {
      ...
      if (conversationIndex !== -1) {
        if (state.conversations[conversationIndex]) {
          ...
          state.conversations[conversationIndex].lastMessageDate =
            message.createdAt;
        }
      }
    },
